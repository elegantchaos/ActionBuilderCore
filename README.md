<!-- Header Generated by ActionBuilderTool 2.2.2 (145) -->
<!-- See https://github.com/elegantchaos/ActionBuilderCore for more details. -->

[![Test results][tests shield]][actions] [![Latest release][release shield]][releases] [![swift 6.2 shield] ![swift 6.3 shield]][swift] ![Platforms: macOS][platforms shield]
...
Swift code that can take a Swift package and create GitHub Actions workflows for it.

The generated workflows build and test the package using GitHub Actions. The main workflow
contains concise jobs which call generated, repository-local reusable workflows for Swift
Package Manager and Xcode builds.
...
To use the tool to generate workflows for your package, you can run the following command
...
The tool writes three files into `.github/workflows`:

- the configured caller workflow, such as `Tests.yml`
- `ActionBuilderSwiftJob.yml` for Swift Package Manager jobs on macOS and Linux
- `ActionBuilderXcodeJob.yml` for simulator-backed Xcode jobs

Test-target source imports are inspected when these files are generated. On supported Swift
versions, the Swift helper emits XCTest and Swift Testing commands only for the frameworks the
package actually uses; older compilers and unrecognized test infrastructure use one combined
`swift test` command.
